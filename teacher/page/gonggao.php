...
$sql = "SELECT name FROM gonggao WHERE head='$head'";
    ...
    if ($result->num_rows > 0) {
        ...
        if(/*$user != "error" && $teacher == "error"*/$name != "error"){
            //echo "发布者：{$user}";
            echo "发布者：{$name}";
...
$sql = "SELECT body,class FROM gonggao WHERE head='$head'";
    mysqli_query($conn,"set names utf8");
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    if ($result->num_rows > 0) {
